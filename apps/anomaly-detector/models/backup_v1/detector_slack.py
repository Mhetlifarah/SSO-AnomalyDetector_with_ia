        ...
        try:
            ...
            if ml_model:
                try:
                    ...
                    if 'avg_interval_sec' in event:
                        ml_context['avg_interval_sec'] = float(event['avg_interval_sec'])
                    if 'n_distinct_clients' in event:
                        ml_context['n_distinct_clients'] = float(event['n_distinct_clients'])

                    is_anomaly, score = ml_model.predict(event, ml_context=ml_context)
                    logger.info(f"ML score: {score:.4f} anomaly={is_anomaly} rule_fired={rule_fired} type={event_type}")
